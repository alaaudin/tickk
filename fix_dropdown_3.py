...
# Replace using string replace first
find_select = """                              <div className="space-y-1.5">
...
repl_select = """                              <div className="space-y-1.5">
                                <label className="text-[10px] font-semibold text-zinc-500 tracking-wider uppercase">CATEGORY</label>
...
if find_select in content:
    content = content.replace(find_select, repl_select)
    print("Replaced with exact string match")
else:
    print("String not found, attempting regex")
    # match select block
    import re
    content = re.sub(r'<select[\s\S]*?</select>', r'<TicketCategoryDropdown value={ticketCategory} onChange={(val) => setTicketCategory(val as any)} />', content, count=1)
